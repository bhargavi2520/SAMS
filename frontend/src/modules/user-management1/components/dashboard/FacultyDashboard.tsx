...
const AttendanceSwitch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <button
    type="button"
    onClick={onChange}
    className={`relative w-10 h-6 rounded-full transition-colors duration-200 outline-none border-2 flex items-center ${checked ? 'bg-green-400 border-green-400' : 'bg-gray-200 border-gray-300'
      }`}
    aria-pressed={checked}
    aria-label={checked ? 'Present' : 'Absent'}
  >
    <span
      className={`absolute left-0 top-0 w-5 h-5 rounded-full bg-white shadow transition-transform duration-200 ${checked ? 'translate-x-4' : 'translate-x-1'
        }`}
    />
  </button>
);
...
const FacultyDashboard = () => {
  ...
  useEffect(() => {
    apiClient.get('/dashboard/faculty')
      .then(res => {
  ...
  const [selectedClass, setSelectedClass] = useState(sectionOptions[0]?.value || '');

  const studentsForSelectedClass = facultyData.find(
  ...
  const studentsForExam = facultyData.find(item => item.section === selectedExamClass)?.students || [];
                  ...
                  <div className="overflow-x-auto">
                    <div className="w-full">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {attendance.length === 0 ? (
                          <div className="col-span-3 text-center text-gray-500 py-4">No students found for this class.</div>
                        ) : (
                          attendance.map((item, idx) => (
                            <div
                              key={item.id}
                              className="grid grid-cols-3 items-center bg-white border rounded-md px-2 py-2 mb-2 shadow-sm"
                            >
                              {/* Name */}
